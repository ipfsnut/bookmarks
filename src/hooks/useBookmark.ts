...
import { API_ENDPOINTS, CONTRACT_ADDRESSES } from '../config/constants';
import BookmarkNFTABI from '../config/abis/BookmarkNFT.json';
import BookmarkVotingABI from '../config/abis/BookmarkVoting.json';

// Define the bookmark interface (same as in BookmarkContext)
...
/**
 * Hook to fetch and manage bookmarks with blockchain-first approach
 */
export function useBookmark() {
   ...
   */
  const fetchBookmarksFromDatabase = async (options: FetchOptions = {}): Promise<Bookmark[]> => {
    try {
  ...
  /**
   * Fetch bookmarks with blockchain-first approach
   */
  const fetchBookmarks = useCallback(async (options: FetchOptions = {}): Promise<Bookmark[]> => {
    ...
    try {
      ...
      const databaseBookmarks = await fetchBookmarksFromDatabase(options);
      
      // Update state
      setBookmarks(databaseBookmarks);
      
      // Update user bookmarks
      if (address) {
        const userBooks = databaseBookmarks.filter(
          bookmark => bookmark.owner_address?.toLowerCase() === address.toLowerCase()
        );
        setUserBookmarks(userBooks);
      }
  ...
  /**
   * Fetch a bookmark by ID with blockchain-first approach
   */
